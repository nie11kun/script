...
def find_intersecting_points(points, normals, line_point, line_direction, min_distance=0.001):
    intersecting_points = []
    num_curves = len(points) // len(curve_points)  # 螺旋曲面中的曲线条数
    for curve_index in range(num_curves):
        curve_start = curve_index * len(curve_points)
        curve_end = (curve_index + 1) * len(curve_points)
        curve_points_segment = points[curve_start:curve_end]
        curve_normals_segment = normals[curve_start:curve_end]
        min_x = np.min(curve_points_segment[:, 0])
        
        for i in range(len(curve_points_segment)):
            normal_line_point = curve_points_segment[i]
            normal_line_direction = curve_normals_segment[i]
            distance = line_to_line_distance(normal_line_point, normal_line_direction, line_point, line_direction)
            if distance < min_distance:
                intersecting_points.append(normal_line_point)
                if normal_line_point[0] <= min_x:
                    break  # 如果是该曲线中 x 坐标最小的点，则结束循环
