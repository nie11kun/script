...
./CloudflareST -url https://niekun.net/cloud/other/cfspeedtest.png -p 0 -f "/Users/marconie/Development/VPN/cloudflare/POWER LINE - 中国 香港.txt" -o "result-powerline.csv"
...
awk -F, '/\./ {if($6 != "0.00") {print $1}}' result-powerline.csv | while read output; do
    ping -c 1 "$output" >/dev/null
    if [ $? -eq 0 ]; then
        echo "$output is ok"
    fi
